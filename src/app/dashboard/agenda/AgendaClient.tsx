...
export default function AgendaClient({ initialAppointments, patients, services, tenantSettings }: { initialAppointments: Appointment[], patients: Patient[], services: Service[], tenantSettings: TenantSettings }) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    // Fix for different browser interpretetions of Date constructor
              ...
              return (
                <div key={appointment.id} className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 py-6 hover:bg-slate-50/50 transition-all rounded-2xl px-2 sm:px-6 group">
                  <div className="flex items-center gap-4 sm:gap-6">
                    <div className={`w-16 h-16 rounded-2xl flex flex-col items-center justify-center shrink-0 border shadow-inner ${appointment.status === 'COMPLETED' ? 'bg-slate-50 border-slate-200 text-slate-500' : appointment.status === 'CANCELED' ? 'bg-rose-50 border-rose-200 text-rose-500' : 'bg-gradient-to-b from-teal-50 to-emerald-50/30 text-teal-700 border-teal-100/50'}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 sm:gap-4 mt-2 text-sm font-semibold text-slate-500 flex-wrap">
                        <span className="flex items-center gap-1.5 bg-slate-100 px-2.5 py-1 rounded-lg text-slate-600"><Clock className="w-4 h-4 text-slate-400" /> {format(new Date(appointment.date), 'HH:mm')}h</span>
                        {appointment.service && <span className="flex items-center gap-1.5 text-teal-600 bg-teal-50 px-2.5 py-1 rounded-lg border border-teal-100 font-bold">{appointment.service.name}</span>}
                        ...
                        {appointment.status === 'SCHEDULED' && <span className="flex items-center gap-1.5 text-teal-700 bg-teal-50 px-2.5 py-1 rounded-lg border border-teal-200">Agendado</span>}
                      </div>
                    </div>
                  </div>

                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-4 xl:mt-0 w-full xl:w-auto">
            ...
            <form onSubmit={handleAdd} className="p-6 space-y-5">
              ...
              {!editAppId && (
                <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-100">
                  ...
                  {recurring !== "NONE" && (
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-1.5">Vezes</label>
                      <input 
                        type="number" 
                        min="2" max="24"
                        value={occurrences}
